...
describe('PaymentsController', () => {
	...
	describe('Stripe Integration Logic', () => {
		it('should create checkout session correctly', async () => {
			...
			mockStripe.checkout.sessions.create.mockResolvedValue(mockStripeData.checkoutSession);
			
			const session = await mockStripeIntegrationLogic.createCheckoutSession(user, priceId);
		...
		it('should retrieve checkout session correctly', async () => {
			...
			mockStripe.checkout.sessions.retrieve.mockResolvedValue(mockStripeData.checkoutSession);
			
			const session = await mockStripeIntegrationLogic.retrieveCheckoutSession(sessionId);
		...
		it('should update subscription correctly', async () => {
			...
			const subscription = await mockStripeIntegrationLogic.updateSubscription(subscriptionId, data);
			
			expect(subscription).toEqual({ id: subscriptionId });
			expect(mockStripe.subscriptions.update).toHaveBeenCalledWith(subscriptionId, data);
		});

		it('should list customers correctly', async () => {
			const email = 'user@example.com';
			
			mockStripe.customers.list.mockResolvedValue({ data: [mockStripeData.customer] });
		...
		it('should list invoices correctly', async () => {
			...
			mockStripe.invoices.list.mockResolvedValue({ data: [mockStripeData.invoice] });
			
			const invoices = await mockStripeIntegrationLogic.listInvoices(customerId, limit);
	...
	describe('Premium Activation Logic', () => {
		it('should identify premium deluxe correctly', () => {
			...
			expect(mockPremiumActivationLogic.isPremiumDeluxe('price_1Qt63NCOLiDbHvw13PRhpenX')).toBe(false);
		});
		...
		it('should create deluxe message correctly', () => {
			...
			const message = mockPremiumActivationLogic.createDeluxeMessage(clerkUserId);
		...
		it('should update Clerk metadata correctly', async () => {
			...
			await mockPremiumActivationLogic.updateClerkMetadata(clerkUserId, isPremium, premiumDeluxe);
			
			expect(mockFetch).toHaveBeenCalledWith(`https://api.clerk.com/v1/users/${clerkUserId}`, {
				method: 'PATCH',
				headers: {
	...
	describe('Payment History Logic', () => {
		...
		it('should get payment history correctly', async () => {
			...
			mockPrisma.payment.findMany.mockResolvedValue(mockPaymentData.paymentHistory);
		...
		it('should format Stripe payment correctly', () => {
			const invoice = mockStripeData.invoice;
			const formattedPayment = mockPaymentHistoryLogic.formatStripePayment(invoice);
			...
			expect(formattedPayment.period).toEqual(new Date(invoice.period_start * 1000));
	...
	describe('Auto-Renewal Logic', () => {
		it('should cancel auto-renewal without Stripe subscription', async () => {
			...
			expect(result.message).toBe('Автопродление подписки отключено (без Stripe).');
		...
		it('should cancel auto-renewal with Stripe subscription', async () => {
			...
			mockStripe.subscriptions.update.mockResolvedValue({ id: user.stripeSubscriptionId });
			mockPrisma.user.update.mockResolvedValue(user);
			...
			expect(mockStripe.subscriptions.update).toHaveBeenCalledWith(user.stripeSubscriptionId, {
	...
	describe('Controller Logic', () => {
		it('should process createCheckoutSession request successfully', async () => {
			...
			expect(mockStripe.prices.retrieve).toHaveBeenCalledWith('price_1Qt5J0COLiDbHvw1IQNl90uU');
			expect(mockStripe.checkout.sessions.create).toHaveBeenCalledWith({
				payment_method_types: ['card'],
				mode: 'subscription',
				...
				success_url: 'https://worknow.co.il/success?session_id={CHECKOUT_SESSION_ID}',
				cancel_url: 'https://worknow.co.il/cancel',
				metadata: { clerkUserId: 'user_123', priceId: 'price_1Qt5J0COLiDbHvw1IQNl90uU' },
			});
		...
		it('should process createCheckoutSession request with missing clerkUserId', async () => {
			...
			expect(res.json).toHaveBeenCalledWith({ error: 'clerkUserId is required' });
		});
		...
		it('should process createCheckoutSession request with user without email', async () => {
			...
			mockPrisma.user.findUnique.mockResolvedValue(mockUserData.userWithoutEmail);
		...
		it('should process createCheckoutSession request with invalid priceId', async () => {
			const req = mockRequestResponseLogic.buildRequest({ priceId: 'invalid_price' });
			const res = mockRequestResponseLogic.buildResponse();
		...
		it('should process createCheckoutSession request with Stripe error', async () => {
			...
			mockPrisma.user.findUnique.mockResolvedValue(mockUserData.validUser);
			mockStripe.prices.retrieve.mockResolvedValue(mockStripeData.validPrice);
			mockStripe.checkout.sessions.create.mockRejectedValue(mockErrors.stripeError);
			
			await mockControllerLogic.processCreateCheckoutSession(req, res);
			
			expect(console.error).toHaveBeenCalledWith('❌ Ошибка при создании Checkout Session:', mockErrors.stripeError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка при создании сессии' });
		});
		...
		it('should process activatePremium request successfully', async () => {
			...
			expect(console.log).toHaveBeenCalledWith('🔍 activatePremium called with sessionId:', 'cs_test_123');
			...
			expect(mockServices.sendTelegramNotification).toHaveBeenCalledWith(userWithJobs, []);
			...
			expect(mockServices.sendProWelcomeEmail).toHaveBeenCalledWith('user@example.com', 'John Doe');
		...
		it('should process activatePremium request with unpaid session', async () => {
			...
			mockStripe.checkout.sessions.retrieve.mockResolvedValue(mockStripeData.unpaidSession);
		...
		it('should process activatePremium request with premium deluxe', async () => {
			...
			expect(mockServices.sendPremiumDeluxeWelcomeEmail).toHaveBeenCalledWith('user@example.com', 'John Doe');
		});
		...
		it('should process activatePremium request with error', async () => {
			...
			mockStripe.checkout.sessions.retrieve.mockRejectedValue(mockErrors.stripeError);
			
			await mockControllerLogic.processActivatePremium(req, res);
			
			expect(console.error).toHaveBeenCalledWith('❌ Ошибка активации премиума:', mockErrors.stripeError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка активации премиума' });
		});
		...
		it('should process cancelAutoRenewal request successfully', async () => {
			...
			const userWithStripe = { ...mockUserData.validUser, stripeSubscriptionId: 'sub_123', isAutoRenewal: true };
		...
		it('should process cancelAutoRenewal request with user not found', async () => {
			...
			expect(console.error).toHaveBeenCalledWith('❌ Ошибка: пользователь не найден.');
			expect(res.status).toHaveBeenCalledWith(404);
		...
		it('should process cancelAutoRenewal request with already disabled', async () => {
			...
			const userWithoutAutoRenewal = { ...mockUserData.validUser, isAutoRenewal: false };
			mockPrisma.user.findUnique.mockResolvedValue(userWithoutAutoRenewal);
			
			await mockControllerLogic.processCancelAutoRenewal(req, res);
			
			expect(console.warn).toHaveBeenCalledWith('⚠️ Автопродление уже отключено.');
			expect(res.status).toHaveBeenCalledWith(400);
			expect(res.json).toHaveBeenCalledWith({ error: 'Автопродление уже отключено' });
		});
		...
		it('should process cancelAutoRenewal request without Stripe subscription', async () => {
			...
			const userWithoutStripe = { ...mockUserData.validUser, stripeSubscriptionId: null, isAutoRenewal: true };
		...
		it('should process addPaymentHistory request with error', async () => {
			...
			expect(console.error).toHaveBeenCalledWith('Ошибка при добавлении платежа:', mockErrors.databaseError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка при добавлении платежа' });
		});
		...
		it('should process getPaymentHistory request with missing clerkUserId', async () => {
			...
			expect(res.status).toHaveBeenCalledWith(400);
			expect(res.json).toHaveBeenCalledWith({ error: 'clerkUserId обязателен' });
		});
		...
		it('should process getPaymentHistory request with error', async () => {
			...
			expect(console.error).toHaveBeenCalledWith('Ошибка при получении истории платежей:', mockErrors.databaseError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка при получении истории платежей' });
		});
		...
		it('should process renewAutoRenewal request successfully', async () => {
			...
			expect(mockPrisma.user.findUnique).toHaveBeenCalledWith({ where: { clerkUserId: 'user_123' } });
			expect(mockPrisma.user.update).toHaveBeenCalledWith({
				where: { clerkUserId: 'user_123' },
				data: { isAutoRenewal: true },
			});
		...
		it('should process renewAutoRenewal request with already enabled', async () => {
			...
			expect(res.status).toHaveBeenCalledWith(400);
			expect(res.json).toHaveBeenCalledWith({ error: 'Автопродление уже включено' });
		});
		...
		it('should process renewAutoRenewal request with error', async () => {
			...
			expect(console.error).toHaveBeenCalledWith('❌ Ошибка при включении автопродления:', mockErrors.databaseError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка при включении автопродления' });
		});
		...
		it('should process getStripePaymentHistory request successfully', async () => {
			...
			const userWithCustomer = { ...mockUserData.validUser, stripeCustomerId: null };
			...
			expect(res.json).toHaveBeenCalledWith({
				payments: expect.arrayContaining([expect.objectContaining({
					id: 'in_123',
					amount: 10,
					currency: 'usd',
					status: 'paid',
		...
		it('should process getStripePaymentHistory request with no customer', async () => {
			...
			const userWithoutCustomer = { ...mockUserData.validUser, stripeCustomerId: null };
		...
		it('should process getStripePaymentHistory request with error', async () => {
			...
			expect(console.error).toHaveBeenCalledWith('Ошибка при получении истории Stripe:', mockErrors.databaseError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка при получении истории Stripe' });
		});
		...
		it('should handle controller errors', () => {
			...
			mockControllerLogic.handleControllerError(error, res, 'creating checkout session');
			
			expect(console.error).toHaveBeenCalledWith('❌ Error creating checkout session:', mockErrors.databaseError);
			expect(res.status).toHaveBeenCalledWith(500);
			expect(res.json).toHaveBeenCalledWith({ error: 'Ошибка при creating checkout session' });
		});
		...
		it('should validate controller input', () => {
			...
			expect(mockControllerLogic.validateControllerInput(invalidRequest)).toBe(true); // Empty body still has body property
		});
	});
});
