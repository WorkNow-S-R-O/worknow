...
describe('NewsletterController', () => {
	...
	describe('Validation Logic', () => {
		it('should validate email correctly', () => {
			expect(
				mockValidationLogic.validateEmail('user@example.com').isValid,
			).toBe(true);
			expect(mockValidationLogic.validateEmail('').isValid).toBe(false);
			expect(mockValidationLogic.validateEmail('invalid-email').isValid).toBe(
				false,
			);
			expect(mockValidationLogic.validateEmail('').error).toBe(
				'Email is required',
			);
			expect(mockValidationLogic.validateEmail('invalid-email').error).toBe(
		...
		it('should validate subscription data correctly', () => {
			...
			expect(
				mockValidationLogic.validateSubscriptionData(validData).isValid,
			).toBe(true);
			expect(
				mockValidationLogic.validateSubscriptionData(invalidData).isValid,
			).toBe(false);
		});
		...
		it('should validate verification data correctly', () => {
			...
			expect(
				mockValidationLogic.validateVerificationData(validData).isValid,
			).toBe(true);
			expect(
				mockValidationLogic.validateVerificationData(invalidData).isValid,
			).toBe(false);
			expect(
				mockValidationLogic.validateVerificationData(invalidData).error,
			).toBe('Email and verification code are required');
		});

		it('should validate preferences data correctly', () => {
			const data = mockSubscriptionData.validSubscriptionData;
			expect(mockValidationLogic.validatePreferencesData(data).isValid).toBe(
				true,
			);
		});

		it('should validate email format correctly', () => {
			expect(mockValidationLogic.validateEmailFormat('user@example.com')).toBe(
				true,
			);
			expect(mockValidationLogic.validateEmailFormat('invalid-email')).toBe(
				false,
			);
		...
		it('should validate email required correctly', () => {
			expect(
				mockValidationLogic.validateEmailRequired('user@example.com'),
			).toBe(true);
		...
		it('should validate verification code correctly', () => {
			...
			expect(mockValidationLogic.validateVerificationCode('abcdef')).toBe(
				false,
			);
		});
		...
		it('should validate employment correctly', () => {
			expect(
				mockValidationLogic.validateEmployment(['Full-time', 'Part-time']),
			).toBe(true);
	...
	describe('Email Processing Logic', () => {
		it('should process email correctly', () => {
			expect(
				mockEmailProcessingLogic.processEmail('  USER@EXAMPLE.COM  '),
			).toBe('user@example.com');
			expect(mockEmailProcessingLogic.processEmail('user@example.com')).toBe(
				'user@example.com',
			);
		});
		...
		it('should validate email format correctly', () => {
			expect(
				mockEmailProcessingLogic.validateEmailFormat('user@example.com'),
			).toBe(true);
			expect(
				mockEmailProcessingLogic.validateEmailFormat('invalid-email'),
			).toBe(false);
		});

		it('should format email for storage correctly', () => {
			expect(
				mockEmailProcessingLogic.formatEmailForStorage('  USER@EXAMPLE.COM  '),
			).toBe('user@example.com');
		});
	...
	describe('Verification Processing Logic', () => {
		...
		it('should process verification code correctly', () => {
			expect(
				mockVerificationProcessingLogic.processVerificationCode(123456),
			).toBe('123456');
			expect(
				mockVerificationProcessingLogic.processVerificationCode('123456'),
			).toBe('123456');
		...
		it('should validate verification code correctly', () => {
			expect(
				mockVerificationProcessingLogic.validateVerificationCode('123456'),
			).toBe(true);
			expect(
				mockVerificationProcessingLogic.validateVerificationCode('000000'),
			).toBe(true);
			expect(
				mockVerificationProcessingLogic.validateVerificationCode('12345'),
			).toBe(false);
			expect(
				mockVerificationProcessingLogic.validateVerificationCode('abcdef'),
			).toBe(false);
		});
		...
		it('should handle verification error correctly', () => {
			...
			const result =
				mockVerificationProcessingLogic.handleVerificationError(error);
		...
		it('should handle verification success correctly', () => {
			const code = '123456';
			const result =
				mockVerificationProcessingLogic.handleVerificationSuccess(code);
	...
	describe('Subscription Processing Logic', () => {
		it('should process subscription data correctly', () => {
			const data = mockSubscriptionData.validSubscriptionData;
			const result =
				mockSubscriptionProcessingLogic.processSubscriptionData(data);
		...
		it('should process subscription response correctly', () => {
			const subscriber = mockSubscriberData.validSubscriber;
			const result =
				mockSubscriptionProcessingLogic.processSubscriptionResponse(subscriber);
			...
			expect(result.preferredEmployment).toEqual(
			...
			expect(result.preferredDocumentTypes).toEqual(
		...
		it('should handle subscription error correctly', () => {
			...
			const result =
				mockSubscriptionProcessingLogic.handleSubscriptionError(error);
		...
		it('should handle subscription success correctly', () => {
			const subscriber = mockSubscriberData.validSubscriber;
			const result =
				mockSubscriptionProcessingLogic.handleSubscriptionSuccess(subscriber);
	...
	describe('Controller Logic', () => {
		it('should process subscribeToNewsletter request successfully', async () => {
			...
			mockNewsletterServices.sendInitialCandidatesToNewSubscriber.mockResolvedValue(
				true,
			);
			...
			expect(
				mockNewsletterServices.sendInitialCandidatesToNewSubscriber,
			).toHaveBeenCalledWith(mockSubscriberData.validSubscriber);
		...
		it('should process subscribeToNewsletter request with existing subscriber', async () => {
			...
			mockPrisma.newsletterSubscriber.findUnique.mockResolvedValue(
				mockSubscriberData.existingSubscriber,
			);
		...
		it('should process unsubscribeFromNewsletter request successfully', async () => {
			...
			mockPrisma.newsletterSubscriber.delete.mockResolvedValue(
				mockSubscriberData.validSubscriber,
			);
		...
		it('should process sendNewsletterVerificationCode request successfully', async () => {
			...
			expect(mockNewsletterServices.storeVerificationCode).toHaveBeenCalledWith(
				'user@example.com',
				expect.any(String),
			);
			expect(console.log).toHaveBeenCalledWith(
			...
			expect(mockNewsletterServices.sendVerificationCode).toHaveBeenCalledWith(
				'user@example.com',
				expect.any(String),
			);
			expect(console.log).toHaveBeenCalledWith(
		...
		it('should process verifyNewsletterCode request successfully', async () => {
			const req = mockRequestResponseLogic.buildRequest(
				mockVerificationData.verificationRequest,
			);
			const res = mockRequestResponseLogic.buildResponse();

			const subscriberWithCorrectEmail = {
				...mockSubscriberData.validSubscriber,
				email: 'verify@example.com',
			};
			mockNewsletterServices.verifyCode.mockResolvedValue({ valid: true });
			mockPrisma.newsletterSubscriber.create.mockResolvedValue(
				subscriberWithCorrectEmail,
			);
			mockNewsletterServices.sendInitialCandidatesToNewSubscriber.mockResolvedValue(
				true,
			);
			...
			expect(mockNewsletterServices.verifyCode).toHaveBeenCalledWith(
				'verify@example.com',
				'123456',
			);
			expect(mockPrisma.newsletterSubscriber.create).toHaveBeenCalledWith({
			...
			expect(
				mockNewsletterServices.sendInitialCandidatesToNewSubscriber,
			).toHaveBeenCalledWith(subscriberWithCorrectEmail);
			expect(res.json).toHaveBeenCalledWith({
				success: true,
				message: 'Successfully subscribed to newsletter',
				subscriber: {
		...
		it('should process verifyNewsletterCode request with missing fields', async () => {
			const req = mockRequestResponseLogic.buildRequest(
				mockVerificationData.verificationRequestMissingFields,
			);
			const res = mockRequestResponseLogic.buildResponse();
		...
		it('should process verifyNewsletterCode request with invalid code', async () => {
			const req = mockRequestResponseLogic.buildRequest(
				mockVerificationData.verificationRequestWithInvalidCode,
			);
			const res = mockRequestResponseLogic.buildResponse();

			mockNewsletterServices.verifyCode.mockResolvedValue({
				valid: false,
