...
// Mock window object for React DOM operations
const mockDocument = {
	activeElement: null,
	body: {
		appendChild: vi.fn(),
...
// Mock global window and document for React DOM
global.window = window;
global.document = mockDocument;

// Mock React DOM createRoot to avoid container issues
...
describe('NewsletterAdmin Component', () => {
	...
	beforeEach(() => {
		...
		// Ensure window is properly mocked for React DOM operations
		global.window = {
			location: { href: 'http://localhost:3000' },
			alert: vi.fn(),
			navigate: vi.fn(),
	...
	afterEach(() => {
		// Clear any pending timers and async operations
		vi.clearAllTimers();
		// Reset window mock
		global.window = {
			location: { href: 'http://localhost:3000' },
