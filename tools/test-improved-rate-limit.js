...
async function testImprovedRateLimitHandling() {
    console.log("🧪 Testing improved rate limit handling for AI job title generation...\n");
    console.log("📊 Using conservative settings: 3 requests per minute, 20 seconds between requests\n");
    ...
    console.log(`📝 Testing ${testDescriptions.length} job descriptions with conservative rate limiting...\n`);
    ...
    for (let i = 0; i < testDescriptions.length; i++) {
        ...
        if (i < testDescriptions.length - 1) {
            console.log("   ⏳ Waiting 25 seconds before next test (conservative rate limiting)...");
