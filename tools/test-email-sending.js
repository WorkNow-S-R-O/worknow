...
async function testEmailSending() {
	...
	try {
		...
		console.log(
			'📧 This should trigger the server to send an email via Resend',
		);
		...
		if (response.data.success) {
			...
			console.log(
				'📧 The email should be from: WorkNow <onboarding@resend.dev>',
			);
			console.log('📧 Subject: WorkNow - Код подтверждения');
			console.log('📧 If you received the email, the system is working!');
			console.log(
				'📧 If you did NOT receive the email, there is an issue with the server logs or email sending',
			);
		}
	} catch (error) {
