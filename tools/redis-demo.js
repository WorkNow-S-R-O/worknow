...
async function demonstrateRedisPerformance() {
	...
	console.log(
		`   Status: ${firstRequest.status} | Time: ${firstRequest.time}ms\n`,
	);
	...
	console.log(
		`   Status: ${secondRequest.status} | Time: ${secondRequest.time}ms\n`,
	);
	...
	console.log(
		`   Status: ${thirdRequest.status} | Time: ${thirdRequest.time}ms\n`,
	);

	// Calculate performance improvement
	const improvement = (
		((firstRequest.time - secondRequest.time) / firstRequest.time) *
		100
	).toFixed(1);
	...
	try {
		...
		console.log(
			`💾 Memory Used: ${health.redis.memory.split('used_memory_human:')[1]?.split('\n')[0] || 'N/A'}`,
		);
	} catch (error) {
