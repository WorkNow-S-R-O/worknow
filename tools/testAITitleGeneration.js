...
const testCases = [
	{
		description:
			'! ХОЛОН /КУХНЯ В ОФИСЕ ПОВАР Вс-Чт 06.00-16.00 Оплата: от 45-50 шек/час Проезд оплачиваем Питание есть Требуется опыт работы и знание иврита Требуются девушки и мужчины !! ОФИЦИАЛЬНОЕ ТРУДОУСТРОЙСТВО!!! ВАКАНСИИ ПО ВСЕМУ ИЗРАИЛЮ ! Звоните +972535875519 или пишите WhatsApp +972536232595',
		expected: 'Повар в офисе',
		context: { city: 'Холон', salary: '45-50' },
	},
	{
		description:
			'Бней Брак Требуются сотрудники с ивритом Работа: раскладка товара и работа на кассе Оплата: 45 шек/час График: — Магазин работает с 6:00 до 22:30 — Пятница: 7:00-15:00— Работа в две смены Требования: — Знание иврита обязательно — Ответственность, аккуратность ОФИЦИАЛЬНОЕ ТРУДОУСТРОЙСТВО +972539396070 ЗВОНИ ЛИБО ПИШИ В WHATSAPP',
		expected: 'Продавец-консультант',
		context: { city: 'Бней Брак', salary: '45' },
	},
	{
		description:
			'Требуется уборщик в офис в Тель-Авиве. Работа 5 дней в неделю. Оплата 40 шек/час. Знание иврита не обязательно.',
		expected: 'Уборщик офиса',
		context: { city: 'Тель-Авив', salary: '40' },
	},
	{
		description:
			'Ищем грузчика на склад в Хайфе. Работа с 8:00 до 17:00. Оплата 35 шек/час. Требуется физическая выносливость.',
		expected: 'Грузчик',
		context: { city: 'Хайфа', salary: '35' },
	},
	{
		description:
			'Нужен водитель с правами категории B. Доставка по Израилю. Оплата 50 шек/час. Опыт работы обязателен.',
		expected: 'Водитель',
		context: { salary: '50' },
	},
	{
		description:
			'Требуется официант в ресторан в Иерусалиме. Работа в вечерние смены. Оплата 45 шек/час. Знание иврита обязательно.',
		expected: 'Официант',
		context: { city: 'Иерусалим', salary: '45' },
	},
	{
		description:
			'Ищем продавца в магазин одежды в Нетании. Работа с 9:00 до 19:00. Оплата 40 шек/час. Опыт в продажах приветствуется.',
		expected: 'Продавец-консультант',
		context: { city: 'Нетания', salary: '40' },
	},
	{
		description:
			'Требуется кассир в торговый центр в Ашдоде. Работа в две смены. Оплата 42 шек/час. Знание иврита обязательно.',
		expected: 'Кассир',
		context: { city: 'Ашдод', salary: '42' },
	},
	{
		description:
			'Нужен электрик для работы в строительной компании. Опыт работы обязателен. Оплата 60 шек/час. Работа по всему Израилю.',
		expected: 'Электрик',
		context: { salary: '60' },
	},
	{
		description:
			'Ищем сантехника для работы в жилых домах. Опыт работы обязателен. Оплата 55 шек/час. Работа в Тель-Авиве и окрестностях.',
		expected: 'Сантехник',
		context: { salary: '55' },
	},
];
...
async function testAITitleGeneration() {
	...
	for (const testCase of testCases) {
		try {
			...
			const isSuccess =
				result.title.toLowerCase().includes(testCase.expected.toLowerCase()) ||
...
async function testSingleJob(description, context = {}) {
	...
	try {
		...
		console.log(
			`   - Has specific keywords: ${result.analysis.hasSpecificKeywords}`,
		);
		console.log(`   - Has location: ${result.analysis.hasLocation}`);
		console.log(`   - Has salary: ${result.analysis.hasSalary}`);
		console.log(
			`   - Has language requirement: ${result.analysis.hasLanguageRequirement}`,
		);
		console.log(
			`   - Has experience requirement: ${result.analysis.hasExperienceRequirement}`,
		);
...
async function testBatchProcessing() {
	...
	try {
		...
		results.forEach((job, index) => {
			console.log(
				`${index + 1}. "${job.title}" (${job.titleMethod}, confidence: ${job.titleConfidence.toFixed(2)})`,
			);
		});
	} catch (error) {
