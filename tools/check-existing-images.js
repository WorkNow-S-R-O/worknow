...
async function checkExistingImages() {
	try {
		console.log(
			'🔍 Checking existing images in database for moderation violations...\n',
		);
		...
		for (let i = 0; i < jobsWithImages.length; i++) {
			...
			console.log(
				`🔍 Checking image ${i + 1}/${jobsWithImages.length}: Job #${job.id} - "${job.title}"`,
			);
			...
			try {
				...
				if (moderationResult.isApproved) {
					...
					console.log(
						`   ❌ VIOLATION - ${moderationResult.detectedIssues.criticalViolations.map((v) => v.name).join(', ')}`,
					);
				}
		...
		if (violations.length > 0) {
			...
			violations.forEach((violation) => {
				...
				console.log(
					`Violations: ${violation.moderationResult.detectedIssues.criticalViolations.map((v) => `${v.name} (${v.confidence}%)`).join(', ')}`,
				);
				console.log('');
			});
		}
