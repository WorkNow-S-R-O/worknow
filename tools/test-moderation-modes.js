...
import { moderateImage, validateRekognitionConfig } from '../apps/api/services/imageModerationService.js';
...
async function testSimplifiedModeration() {
  try {
    ...
    console.log(`   Monitor Only: ${currentConfig.monitorOnly.slice(0, 5).join(', ')}... (${currentConfig.monitorOnly.length} total)\n`);
    ...
    if (currentConfig.monitorOnly.length > 10) {
      console.log(`     ... and ${currentConfig.monitorOnly.length - 10} more items`);
    }
    ...
    try {
      ...
      console.log(`   Status: ${result.isApproved ? '✅ APPROVED' : '❌ REJECTED'}`);
      console.log(`   Confidence: ${result.confidence}%`);
      
      if (result.detectedIssues.criticalViolations.length > 0) {
        console.log(`   Critical Violations: ${result.detectedIssues.criticalViolations.map(v => `${v.name} (${v.confidence}%)`).join(', ')}`);
      }
    ...
    console.log('   This system is designed for job advertisements and work-related content.');
    console.log('   Only blocks: Porn, Violence, Hate Symbols (with 90%+ confidence)');
    console.log('   Allows: Tools, kitchen items, construction equipment, alcohol, tobacco, etc.');
    ...
    console.log('   🚫 Clear violent content (fighting, weapons in violent context)');
    console.log('   🚫 Hate speech symbols and content');
    console.log('   Note: Only blocked with 90%+ confidence to avoid false positives\n');
