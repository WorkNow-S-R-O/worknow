...
import { sendInitialCandidatesToNewSubscriber, checkAndSendNewCandidatesNotification } from '../apps/api/services/candidateNotificationService.js';
  ...
  try {
    ...
    if (testSubscriber) {
      ...
      try {
        ...
        console.error('❌ Error testing initial candidates email:', error.message);
      }
    } else {
    ...
    console.log(`   - Next notification at: ${Math.ceil((currentCandidateCount + 1) / 3) * 3} candidates`);
    console.log(`   - Candidates until next notification: ${3 - (currentCandidateCount % 3)}`);
