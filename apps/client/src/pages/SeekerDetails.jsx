...
export default function SeekerDetails() {
  ...
  if (loading) return (
    <>
      <Helmet>
        <title>{content.seekerProfileLoading.value || "Загрузка..."} - {content.seekerProfileTitle.value}</title>
      </Helmet>
  ...
  if (!seeker || !seeker.description) return (
    <>
      <Helmet>
        <title>{content.seekerProfileNotFound.value || "Не найдено"} - {content.seekerProfileTitle.value}</title>
      </Helmet>
  ...
  return (
    <>
      <Helmet>
        <title>
          {`${seeker.name} - ${content.seekerProfileTitle.value}`}
        </title>
      ...
      <div className="container" style={{ maxWidth: 600, paddingTop: '100px' }}>
        ...
        <div className="text-muted mb-2">{content.seekerProfilePublished.value.replace('{{days}}', daysAgo).replace('{{date}}', formattedDate)}</div>
        <h3 className="mb-2">
        ...
        {isPremium && (
          <div className="mb-2">
            <strong>{content.seekerProfileNote.value}:</strong> {seeker.note || ''}
          </div>
        )}
        {!isPremium && (
          <div className="mb-2">
        ...
        <button className="btn btn-dark mt-2" onClick={() => window.print()}>{content.seekerProfilePrint.value}</button>
        
                {seekerIds && returnToPage && (
        <div className="mt-4">
          <div className="text-muted mb-2">
            {content.seekerProfileOf.value.replace('{{current}}', globalIndex + 1).replace('{{total}}', seekerIds.length)}
