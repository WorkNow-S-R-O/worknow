...
const NewsletterAdmin = () => {
  ...
  const sendTestNewsletter = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/newsletter/send`, {
        ...
        customMessage: customMessage || 'Это тестовая рассылка для проверки функциональности.'
      });
  ...
  const checkAndSendAutomated = async () => {
    setSending(true);
    try {
      const response = await axios.post(`${API_URL}/api/newsletter/check-and-send`);
            ...
            <div className="card-body">
              ...
              <div className="d-grid gap-2">
                ...
                <button
                  ...
                  disabled={sending}
                >
                  {sending ? 'Проверка...' : 'Проверить и отправить автоматическую рассылку'}
                </button>
