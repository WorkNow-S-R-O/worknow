...
function MobileCityModal({
	...
	const filteredOtherCities = otherCities.filter((city) =>
		(city.label || city.name).toLowerCase().includes(search.toLowerCase()),
	);
	if (!show) return null;
	...
	return createPortal(
		<div
			...
			style={{
				display: 'block',
				background: isMobile ? '#fff' : 'rgba(0,0,0,0.35)',
				zIndex: 4000,
			}}
			...
			>
				<div className="modal-content" style={contentStyle}>
					...
					<div
						...
						style={{
							padding: isMobile ? '0 16px 20px' : '16px',
							flex: 1,
							overflowY: 'auto',
						}}
					>
		...
		const fetchCities = async () => {
			try {
				console.log(
					...
					`${API_URL}/api/cities?lang=${locale}`,
				);
