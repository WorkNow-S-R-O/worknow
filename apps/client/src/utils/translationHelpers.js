...
export const useTranslationHelpers = () => {
	...
	const getGenderLabel = (gender) => {
		if (!gender) return '';
		if (['мужчина', 'male'].includes(gender.toLowerCase()))
			return content.seekerProfileMale.value;
		if (['женщина', 'female'].includes(gender.toLowerCase()))
			return content.seekerProfileFemale.value;
		return gender;
	};
	...
	const getEmploymentLabel = (employment) => {
		...
		if (
			employmentLower === 'полная' ||
			employmentLower === 'full-time' ||
			employmentLower === 'fulltime'
		) {
			return content.employmentPolnaya.value;
		}
		if (
			employmentLower === 'частичная' ||
			employmentLower === 'part-time' ||
			employmentLower === 'parttime'
		) {
	...
	const getCategoryLabel = (category) => {
		...
		if (
			categoryLower === 'бьюти-индустрия' ||
			categoryLower === 'beauty industry'
		) {
	...
	const getDocumentTypeLabel = (documentType) => {
		...
		if (
			docLower === 'теудат зеут' ||
			docLower === 'teudat zehut' ||
			docLower === 'תעודת זהות'
		) {
			return content.documentTeudatZehut.value;
		}
		if (
			docLower === 'рабочая виза' ||
			docLower === 'work visa' ||
			docLower === 'ויזת עבודה'
		) {
	...
	const getCityLabel = (city) => {
		...
		if (
			cityLower === 'ришон-ле-цион' ||
			cityLower === 'rishon lezion' ||
			cityLower === 'ראשון לציון' ||
			cityLower === 'ريشون لتسيون'
		) {
			return content.cityRishonLeTsion.value;
		}
		if (
			cityLower === 'петах-тиква' ||
			cityLower === 'petah tikva' ||
			cityLower === 'פתח תקווה' ||
			cityLower === 'بتاح تكفا'
		) {
		...
		if (
			cityLower === 'рош-ха-аин' ||
			cityLower === 'rosh haayin' ||
			cityLower === 'ראש העין' ||
			cityLower === 'رأس العين'
		) {
