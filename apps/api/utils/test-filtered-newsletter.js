...
async function testFilteredNewsletter() {
  try {
    ...
    subscribers.forEach(sub => {
      ...
      console.log(`    Categories: ${sub.preferredCategories?.join(', ') || 'None'}`);
      console.log(`    Employment: ${sub.preferredEmployment?.join(', ') || 'None'}`);
      console.log(`    Languages: ${sub.preferredLanguages?.join(', ') || 'None'}`);
      console.log(`    Gender: ${sub.preferredGender || 'None'}`);
      console.log(`    Documents: ${sub.preferredDocumentTypes?.join(', ') || 'None'}`);
