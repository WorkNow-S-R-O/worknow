...
async function showCurrentJobTitles() {
    ...
    try {
        ...
        const specificTitles = jobs.filter(job => 
            job.title !== "Общая вакансия" && 
            !job.title.includes("Работник") &&
            job.title.length < 30
        );
        
        const genericTitles = jobs.filter(job => 
            job.title === "Общая вакансия" || 
            job.title.includes("Работник") ||
            job.title.length >= 30
        );
        ...
        console.log(`   Specific titles: ${specificTitles.length} (${((specificTitles.length / jobs.length) * 100).toFixed(1)}%)`);
        console.log(`   Generic titles: ${genericTitles.length} (${((genericTitles.length / jobs.length) * 100).toFixed(1)}%)`);
