...
export const moderateImage = async (imageBuffer) => {
  try {
    ...
    const isInappropriate = detectedInappropriate.length > 0 || detectedPotentiallyInappropriate.length > 0;
    ...
    const result = {
      ...
      analysis: {
        ...
        moderationConfidence: detectedInappropriate.length > 0 
          ? Math.max(...detectedInappropriate.map(label => label.Confidence))
          : 0
      }
    };
...
export const validateRekognitionConfig = () => {
  ...
  const supportedRegions = ['us-east-1', 'us-west-2', 'eu-west-1', 'ap-southeast-2'];
