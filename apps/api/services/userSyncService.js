...
console.log('🔍 UserSyncService - CLERK_SECRET_KEY available:', !!CLERK_SECRET_KEY);

export const syncUserService = async (clerkUserId) => {
  try {
    console.log('🔍 UserSyncService - Starting sync for clerkUserId:', clerkUserId);
    ...
    console.log('🔍 UserSyncService - Clerk API response status:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ UserSyncService - Clerk API error:', response.status, errorText);
