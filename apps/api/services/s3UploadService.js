		...
		try {
			...
			if (!uploadResult.success) {
				if (uploadResult.code === 'CONTENT_REJECTED') {
					throw new Error(
						`Image content violates community guidelines: ${uploadResult.error}`,
					);
				}
	...
	async createJobWithImage(jobData, file, userId) {
		...
		try {
			...
			if (jobCount >= maxJobs) {
				if (isPremium) {
					throw new Error(
						`Вы уже разместили ${MAX_JOBS_PREMIUM_USER} объявлений.`,
					);
				} else {
					throw new Error(
						`Вы уже разместили ${MAX_JOBS_FREE_USER} объявлений. Для размещения большего количества объявлений перейдите на Premium тариф.`,
					);
				}
			}
