...
class AIJobTitleService {
    ...
    static fallbackTitleGeneration(description) {
        ...
        if (lowerDescription.includes('повар') || lowerDescription.includes('кухня')) {
            title = "Повар";
        } else if (lowerDescription.includes('уборщик') || lowerDescription.includes('уборка')) {
            title = "Уборщик";
        } else if (lowerDescription.includes('официант') || lowerDescription.includes('ресторан')) {
            title = "Официант";
        } else if (lowerDescription.includes('грузчик') || lowerDescription.includes('склад')) {
            title = "Грузчик";
        } else if (lowerDescription.includes('водитель') || lowerDescription.includes('доставка')) {
            title = "Водитель";
        } else if (lowerDescription.includes('продавец') || lowerDescription.includes('магазин')) {
            title = "Продавец-консультант";
        } else if (lowerDescription.includes('кассир') || lowerDescription.includes('касса')) {
            title = "Кассир";
        } else if (lowerDescription.includes('строитель') || lowerDescription.includes('строительство')) {
            title = "Строитель";
    ...
    static async batchGenerateAITitles(jobs) {
        console.log(`🤖 Starting AI-powered title generation for ${jobs.length} jobs...`);
        console.log(`📊 Rate limit config: ${RATE_LIMIT_CONFIG.requestsPerMinute} requests per minute`);
        ...
        for (const job of jobs) {
            try {
                ...
                console.error(`❌ Failed to generate title for job ${job.id}:`, error.message);
                errorCount++;
        ...
        console.log(`   Other errors (using fallback): ${errorCount - quotaCount - rateLimitCount}`);
    ...
    static hasSpecificKeywords(description) {
        const specificKeywords = [
            'повар', 'официант', 'грузчик', 'водитель', 'продавец', 
            'кассир', 'уборщик', 'строитель', 'электрик', 'сантехник',
            'маляр', 'курьер', 'программист', 'сиделка', 'няня'
        ];
    ...
    static hasLanguageRequirement(description) {
        const languageKeywords = ['иврит', 'ивритом', 'английский', 'английским', 'русский', 'русским'];
    ...
    static async updateDatabaseWithAITitles() {
        ...
        try {
            ...
            for (const job of jobs) {
                try {
                    ...
                    if (titleData.confidence > 0.7 && titleData.title !== job.title) {
                        ...
                        console.log(`✅ Updated: "${job.title}" → "${titleData.title}" (${titleData.method})`);
                        updatedCount++;
                    } else {
                        console.log(`⏭️  Skipping "${job.title}" - confidence: ${titleData.confidence.toFixed(2)}`);
                        skippedCount++;
                    }
            ...
            console.log(`   Quota errors: ${quotaCount}`);
            console.log(`   Rate limit errors: ${rateLimitCount}`);
            console.log(`   Other errors: ${errorCount - quotaCount - rateLimitCount}`);
